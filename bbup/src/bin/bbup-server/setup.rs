...
pub fn setup(
    ...
    if ServerConfig::exists(&conf_dir) {
    ...
    let archive_root = match opt_archive_root {
        ...
        None => AbstPath::from(input::get("enter archive root (absolute path): ")?),
    };
    // let absolute_archive_root = home_dir.append(&archive_root);
    match archive_root.object_type() {
        Some(ObjectType::Dir) => {}
    ...
    if !list_dir_content(&archive_root)?.is_empty() {
        anyhow::bail!("specified archive root is not empty!");
    }

    ServerConfig::from(server_port, archive_root.clone()).save(&conf_dir)?;
    Archive::new().save_list(&archive_root)?;
